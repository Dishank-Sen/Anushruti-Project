...
The MVP has no child identity, network analytics, camera, or external media dependency. The optional Voice Garden uses a microphone only after an explicit start action; its signal processing is entirely in memory on the device. See [Voice Garden](voice-garden.md) for microphone lifecycle and measurement limits. Local storage is explicitly demo persistence. Treat it as untrusted: validate IDs, counters and values on load. Do not use it for authorization or authoritative completion.
